...
class CarlaDataset(torch.utils.data.Dataset):
    ...
    def get_train_val(self):
        train_scenarios = ['Accident']
        val_scenarios = ['Accident_2']
        train_data, val_data = [], []
        for scenario in train_scenarios:
            train_data.append(os.path.join(self.root_dir, scenario))
        for scenario in val_scenarios:
            val_data.append(os.path.join(self.root_dir, scenario))

        require_data = train_data if self.is_train else val_data
        preload_file = os.path.join(self.root_dir, 'training_data_'+str(self.receptive_field)+'_'+str(self.sequence_length)+'.npy')

        ## load data from provided dataset if it exist, otherwides generate it
        if not os.path.exists(preload_file):
            ...
            for scenario_dir in require_data:
                sensorList = [folder for folder in os.listdir(scenario_dir) if not os.path.isfile(os.path.join(scenario_dir, folder))]
                cameraList = [cam for cam in sensorList if ('Cam' in cam)]

                ## get camera images
                for sensor in cameraList:
                    sensor_dir = os.path.join(scenario_dir, sensor)
                    imageList = sorted([img for img in os.listdir(sensor_dir)], key=get_numeric_part)
                    imageListDict = [sensor_dir + "/" + img for img in imageList]
                    num_seq = len(imageList) - self.sequence_length
                    ...
                    for seq in range(num_seq):
                        ...
                        for i in range(self.receptive_field):
                            ...
                            imgName = f"{str(seq+i)}.png"
                            fronts.append(scenario_dir + "/CamFront/" + imgName)
                            # position
            ...
            preload_dict['left'] = preload_front_left
            preload_dict['right'] = preload_front_right
